close all;
clear;clc;
% the length of a frame is (204*10+128)*8 = 2148*8 = 17344 .=20000
% adjust sim to switch between experiment and simulation
%  sim = 0 simulation | sim = 1 experience | sim == 2 offline
                      sim = 0; cycle = 10;
         port = 'USB0::0x0957::0x1790::MY56271017::0::INSTR'; 
load('data_a\para.mat');    % 加载OOK传输数据的参数这些数据是由发送端程序产生的，接收端提取这些数据用于解调主要参数为过采样倍数，同步序列等
frame_sig_len = 17344;      % 包含同步头在内的 
frame_sig_bits = 128 + 2040;% 一帧数据的bit位数
Tx_stream = bit_stream_m';
test_data = test_data';

ber_bc = zeros(1,cycle);    % 编码前误码率
ber_ac = zeros(1,cycle);    % 编码后误码率
ber_mc = zeros(1,cycle); 

err_bc = zeros(cycle,55);   % 用于在送入8B10B解码前的错误比特数
err_ac = zeros(cycle,55);   % 用于保存RS解码后的错误比特数
err_mc = zeros(cycle,55);   % 用于保存RS编码前这段数据的错误比特数
frame_id = zeros(cycle,55); % 帧头首字节为帧头序号，发端产生了20帧，接收端首先判断帧头，而后查表对比数据BER
for cyc = 0:cycle-1
    ...
    if sim == 0
        ...
        ss_ele_Rx = [Tx_wave',Tx_wave',Tx_wave'];
    elseif sim == 1
...
%     Rx_down = ss_ele_Rx(1:1:end);
    ...
    frame_cnt = floor(length(ss_ele_Rx)/frame_sig_len) - 2;     % 判断接收的这段数据中包含多少段完整帧.
